...
def plot_multi_tier_sims(instance_name,
    ...
    for p in profiles:
        i += 1
        if (any(val == 0 for val in p['tier_history'][t_start+37:T]) and any(val == 4 for val in p['tier_history'][t_start+37:T])):
            print(i)
    ...
    for v in plot_left_axis:
        ...
        if v == 'S0':                
            v_a = ax1.plot(all_st['S0'].T , c=colors[v], linestyle=l_style, linewidth=2, label=label_v, alpha=1 * hide, zorder = 50)
        if v == 'S1':                
        ...
        if v == 'ToIHT_vac':
                if real_new_admission is not None:
                    real_h_plot = ax1.scatter(range(len(real_new_admission_vax)), real_new_admission_vax, color='maroon', label='New hospital admission',zorder=100,s=15)
        
        if v == 'ToIY_moving' :
            if real_new_admission is not None:
                moving_avg_len = config['moving_avg_len']
                real_new_admission = np.array(real_new_admission)
        ...
        if v == 'ToIHT' or v == 'ToICU' or v == 'ToIHT_moving':
            if v == 'ToIHT_moving':
                if real_new_admission is not None:
                    ...
                    real_new_admission = np.array(real_new_admission)
                    T_adm = len(real_new_admission)
                    #real_hosp_moving_avg np.convolve(real_new_admission, np.ones(config['moving_avg_len']), 'valid') / config['moving_avg_len']
                    real_hosp_moving_avg = [(real_new_admission)[i: min(i + moving_avg_len, T_adm)].mean() for i in range(T_adm)]
        ...
        if 'ToIY_moving' in plot_left_axis:
            ...
            for ti in range(len(tiers)):
                ...
                if policy.tier_type == 'constant':
                    u_lb = case_policy_lockdown_thresholds[ti]
                    u_ub = case_policy_lockdown_thresholds_ub[ti]
                   # breakpoint()
                    if u_ub == np.inf:
                        u_ub = max_y_lim_1
                        #breakpoint()
