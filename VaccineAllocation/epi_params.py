...
class EpiSetup:
    ...
    def delta_update_param(self, prev):
        ...
        self.beta = self.beta0 * (1 - prev) + self.beta0 * (self.delta_beta) * prev #increased transmission
    ...
    def omicron_update_param(self, prev):
        ...
        #self.beta = self.beta0 * (self.delta_beta)* (1 - prev) + self.beta0 * (self.delta_beta)*(self.omicron_beta) * prev #increased transmission
        # Kaiming's alternative
    ...
    def variant_update_param(self, prev, seed):
        '''
            Assume an imaginary new variant that is more transmissible.
        '''
        random.seed(seed)
        self.beta = self.beta * (1 - prev) + self.beta * (random.uniform(1.2, 1.8)) * prev #increased transmission
        #self.beta = self.beta * (1 - prev) + self.beta * (self.new_variant_beta) * prev #increased transmission
        #original no random
        #self.beta = self.beta0 * (self.delta_beta)*(self.omicron_beta) * (1 - prev) + self.beta0 * (self.delta_beta)*(self.omicron_beta) * (self.new_variant_beta) * prev #increased transmission
        # Kaiming's alternative
        self.YHR = self.YHR0 * (1 - prev) + self.YHR0 * random.uniform(0.5,2) * prev
        self.YHR_overall =  self.YHR_overall * (1 - prev) + self.YHR_overall * random.uniform(0.5,2) * prev
