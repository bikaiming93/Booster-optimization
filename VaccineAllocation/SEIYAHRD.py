...
def simulate_vaccine(instance, policy, interventions, v_policy, seed=-1, **kwargs):
    ...
    global T_delta, T_omicron, T_variant
    ...
    for t in range(T - 1):
        if t>= T_variant:
            ...
            t1=int(t-T_omicron)
            prev_matrix[t][2]=instance.omicron_prev[t1]
            #prev for omicron variant
            prev_matrix[t][1]=1-prev_matrix[t][2]
            #prev for delta variant
        elif t>= T_delta:
            t1=int(t-T_delta)
            prev_matrix[t][1]=instance.delta_prev[t1]
            #prev for delta variant
            prev_matrix[t][0]=1-prev_matrix[t][1]
            #prev for pre-delta variant
        elif t < T_delta:
            prev_matrix[t][0]=1
            #prev for pre-delta variant
        ...
        for t_idx in range(1):
            ...
            if t >= T_delta:
                for v_groups in v_policy._vaccine_groups:
                    v_groups.delta_update(prev_matrix[t][1])
                epi.delta_update_param(prev_matrix[t][1])
                #epi.delta_update_param2(prev_matrix[t][0],prev_matrix[t][1])
                
               
            #Update epi parameters for omicron:     
            #T_omicron = np.where(np.array(v_policy._instance.cal.calendar) == instance.omicron_start)[0][0]
            if t >= T_omicron:
                epi.omicron_update_param(prev_matrix[t][2])
                for v_groups in v_policy._vaccine_groups:
                    v_groups.omicron_update(prev_matrix[t][2])
